...
function (X, Y, zDimension=1, H, sigma2.T = 1e-12, sigma2.W = 1e12, mySeed=123, epsilon = 1e-6) {
  ...
  phi.init <- list(X = var(as.vector(X)), Y = var(as.vector(Y))) # scalar diagonal noise on the marginals (shared by all features)
  ...
  rownames(phitotal) <- colnames(phitotal) <- c(rownames(X), rownames(Y))
