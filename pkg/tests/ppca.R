...
for (zdim in seq(1, min(xdim, ydim), 1)) {
  ...
  toy <- generate.toydata(N = 100, zDim = zdim, xDim = xdim, yDim = ydim, marginal.covariances = "isotropic")
...
for (zdim in seq(1, xdim, 2)) {
  ...
  toy <- generate.toydata(N = 100, zDim = zdim, xDim = xdim, yDim = xdim, marginal.covariances = "isotropic")
