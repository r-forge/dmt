...
Below is a brief summary of the functionality and installation
instructions.
...
For applications in functional genomics, see \cite{Huopaniemi10,
Lahti09mlsp, Tripathi08}, and the associated
pint\footnote{http://bioconductor.org/packages/release/bioc/html/pint.html}
BioConductor package. This can be installed from within R with
source('http://bioconductor.org/biocLite.R'); biocLite('pint') 
